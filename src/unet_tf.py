...
def arg_parser():
    ...
    parser.add_argument('--resume', type=int, default=0,
                        help='resume the weigths or not')
...
def get_unet0():
    ...
    crop9 = Cropping2D(cropping=((16, 16), (16, 16)), dim_ordering='tf')(conv9)
...
def batch_generator(root, index, RGB, batch_size=32, horizontal_flip=True, vertical_flip=True, swap_axis=True):
...
def _convert_mask(img, RGB):
    ...
    img = img[..., np.newaxis].astype(np.float32)[16:96, 16:96, :]
    return img
...
if __name__ == '__main__':
    ...
    root = '../../MultiClassUNet/dataset/sample_01_25/data_112'
    mask_channel = 1
    batch_size = 32
    nb_epoch = 200
    ...
    if args.resume:
        WEIGHT_PATH = '../checkpoints/0Building/weights.67-1.65.hdf5'
        model = load_model(
            WEIGHT_PATH,
            custom_objects={
